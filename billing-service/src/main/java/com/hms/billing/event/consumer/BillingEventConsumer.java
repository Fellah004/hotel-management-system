...
public class BillingEventConsumer {
    ...
    public void handlePaymentSucceeded(EventEnvelope<Map<String, Object>> event) {
        log.info("Received PaymentSucceeded event: {}", event);
        Map<String, Object> payload = event.getPayload();
        if (payload == null) return;
        ...
        if (resIdObj != null && amountObj != null) {
            try {
                Long reservationId = Long.valueOf(resIdObj.toString());
                BigDecimal amount = new BigDecimal(amountObj.toString());
                billingService.recordPaymentForReservation(reservationId, amount);
                log.info("Successfully recorded payment of {} for reservationId {}", amount, reservationId);
            } catch (Exception e) {
                log.error("Error processing PaymentSucceeded event for payload {}: {}", payload, e.getMessage(), e);
            }
        }
    }
}
