...
import com.hms.guestservice.dto.response.ErrorResponse;
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorResponse> handleDataIntegrityViolation(org.springframework.dao.DataIntegrityViolationException ex, HttpServletRequest request) {
        ...
        ErrorResponse error = ErrorResponse.builder()
                ...
                .status(HttpStatus.CONFLICT.value())
                .error("CONFLICT")
                .message("A database constraint or duplicate entry violation occurred: " + (ex.getMostSpecificCause() != null ? ex.getMostSpecificCause().getMessage() : ex.getMessage()))
                .path(request.getRequestURI())
                .traceId(getTraceId())
                .build();
        return ResponseEntity.status(HttpStatus.CONFLICT).body(error);
